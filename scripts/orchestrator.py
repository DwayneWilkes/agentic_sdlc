...
def cmd_goal(args) -> int:
    ...
    # User confirmed - run based on number of matched work streams
    # (command_type may be SUGGEST for low-confidence matches, but user said yes)
    if len(result.matched_work_streams) == 0:
        print(colored("\nNo work streams to run.", Colors.YELLOW))
        return 0
    ...
    if len(result.matched_work_streams) == 1:
        ...
        # Multiple work streams - run in parallel
